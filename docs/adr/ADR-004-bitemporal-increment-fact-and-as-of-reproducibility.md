...
**2a. Each increment also carries power-sum deltas for exact moments (added per ADR-006's
revision).** Alongside the bin delta, every measurement contributes additive power-sum deltas
— `(Δn, Δx, Δx², Δx³, Δx⁴)` for its raw value `x` — keyed by the same `(instrument_id, channel,
event_hour, arrival_time)`. Because power sums are additive, they ride the *identical* bitemporal
machinery as the bin counts: an as-of query sums the power deltas with `arrival_time ≤` the cutoff,
a horizon filters them the same way, and a correction subtracts the wrong value's powers and adds
the right one's. Moments (mean, variance, skewness, kurtosis) are then computed *exactly* from the
summed powers (ADR-006), with no binning bias — while the bin deltas continue to serve percentiles,
entropy, and drift. Two additive quantities, one append-only fact, one set of as-of/horizon rules.
