...
mass; the generalized Sheppard correction (ADR-006) assumes a density that is smooth and tapers at the
tails.
...
- **Equal-mass binning fails at the zero spike.** If (say) 90% of values are exactly zero, there is no
  ...
  into one coarse bin.
- **Sheppard's correction is unreliable.** ADR-006 already flagged that grouped-data corrections assume a
  smooth density tapering at the tails; a spike at zero with a heavy positive tail violates this directly.
...
- Compute moments/entropy over the wet component (where Sheppard's smoothness assumption is restored),
  reported alongside the dry fraction rather than blended into a single misleading summary.
...
at the zero atom and unreliable moments. Treating the zero atom as just another small bin is rejected — it
